...
import Temptation_power as tpt
...
def compute_utility_grids(x,y,grida,gridz,n,sigma_x,sigma_y,xi,x0):
    ...
    for z in range(len(gridz)):
        for a1 in range(len(grida)):
            for a2 in range(len(grida)):
                for age in range(n):
                    U[z,a1,a2,age] = tpt.calculate_U(x[z,a1,a2,age], sigma_x, x0)
                    T[z,a1,a2,age] = tpt.calculate_T(y[z,a1,a2,age], sigma_y, xi)
...
def stationary_distribution(choice_a, grida, gridz, mass_z, Pi, n):
    ...
    distr_mass = np.zeros(shape = (n, len(gridz), len(grida)))
