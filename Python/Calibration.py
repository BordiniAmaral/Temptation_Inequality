...
@njit
def run_simulations_by_x0(grid_sigx, grid_sigy, x0, data_x, data_y, gridm, xi, consumpt, cons_bin, y_data_sel_sum, W):
    
    print("------------------------ \n   Current x0:",x0,"  \n------------------------\n")
    sqe = np.zeros(shape = (len(grid_sigx), len(grid_sigy)))
    counter = 0
    ...
    for sigx in range(len(grid_sigx)):
        for sigy in range(sigy_start, len(grid_sigy)):
            ...
            if grid_sigy[sigy] > grid_sigx[sigx]:
                sqe[sigx,sigy:] = np.Inf #arbitrary high number
                break
            ...
            for x in range(len(data_x)):
                if consumpt[x] < (x0 + 2):
                    ...
                    x_sim[x] = Newton(f, df, (x0 + 0.0001), 1e-6,int(1e6), consumpt[x], xi, grid_sigx[sigx], grid_sigy[sigy], x0)
            ...
            sqe[sigx,sigy] = calculate_sqe(y_sim, data_x, data_y, x0, gridm, cons_bin, y_data_sel_sum, W)[0]
            
            # Not the first sigy evaluated
            if sigy > sigy_start:
                # Checking if sigy passed minimum
                if sqe[sigx,sigy] > sqe[sigx,sigy-1]:
                    sqe[sigx,:sigy_start] = np.Inf
                    sqe[sigx,(sigy+1):] = np.Inf
...
@njit
def calculate_sqe(y_sim, data_x, data_y, x0, gridm, cons_bin, y_data_sel_sum, W):
...
@njit
def run_x0_simulations(grid_sigx, grid_sigy, grid_x0, data_x, data_y, gridm, xi, W):
    
    # Starting timer
    # start_time = time.time()
    print("Running initial calculations... \n")
    
    sqe = np.zeros(shape = (len(grid_x0), len(grid_sigx), len(grid_sigy)))
    consumpt, cons_bin, y_data_sel_sum = initial_computations(data_x,data_y,gridm)
    
    print("Proceeding to simulations... \n")
    for x0 in range(len(grid_x0)):
        
        sqe[x0,:,:] = run_simulations_by_x0(grid_sigx, grid_sigy, grid_x0[x0], data_x, data_y, gridm, xi, consumpt, cons_bin, y_data_sel_sum, W)
...
@njit
def run_two_steps(grid_sigx, grid_sigy, grid_x0, data_x, data_y, gridm, xi):
    
    W = np.identity(len(gridm))
    
    print("Running first simulation...\n")
    
    sqe_I = run_x0_simulations(grid_sigx, grid_sigy, grid_x0, data_x, data_y, gridm, xi, W)
    ...
    errors_sol = simulate_single(sol_sigx_1, sol_sigy_1, sol_x0_1, data_x, data_y, gridm, xi, W)[1]
    ...
    sqe_W = run_x0_simulations(grid_sigx, grid_sigy, grid_x0, data_x, data_y, gridm, xi, W_new)
...
@njit
def simulate_single(sigx, sigy, x0, data_x, data_y, gridm, xi, W):
    ...
    sqe, m_errors = calculate_sqe(y_sim, data_x, data_y, x0, gridm, cons_bin, y_data_sel_sum, W)
    
    return sqe, m_errors
    
@njit
def initial_computations(data_x, data_y, gridm):
